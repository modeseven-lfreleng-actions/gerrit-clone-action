...
def record_timeout_results(
    ...
    try:
        ...
        # Destinations whose outcome is still unknown.  An ancestor of
        # one of these cannot clean up after itself: see _hold_back.
        held_back = {
            outcome.reserved
            for outcome in outcomes
            if outcome.reserved is not None and outcome.held_back
        }

        for outcome in outcomes:
            if outcome.held_back:
                # The wait is bounded, so a worker can outlast it. Its
                # directory is left alone for now: removing it would race
                # work the worker is still doing. The reservation is held
                # back from every batch-wide release, and the worker's own
                # completion discards a failed clone before giving it up.
                _hold_back(
                    outcome.project,
                    outcome.reserved,
                    generation,
