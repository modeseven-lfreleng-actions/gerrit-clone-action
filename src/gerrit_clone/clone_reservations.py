...
What becomes of a reserved destination when its batch gives up is
decided in :mod:`gerrit_clone.clone_cleanup`.
"""
...
#: The *first* reservation wins, so exactly one clone ever writes to a
#: destination, and it is the only one that can remove it.
#:
...
class TargetOwnedError(RuntimeError):
    """Raised when a clone may not take, or keep, a destination.

    Most often another clone holds it -- in another batch, or in this
    one under a name that resolves to the same path -- and cloning
    anyway would have two writers on one path.  It is also raised for a
    destination that cannot be inspected, and for one whose batch has
    been abandoned.  In every case the clone stands down.
    """
...
def reserved_by_other(path: Path, project: str) -> Path | None:
    ...
    clone is working there the directory is in flux -- ``git clone``
    creates ``.git`` before transferring anything, and its owner's
    cleanup may remove it -- so reporting it as an existing repository
    would describe something about to disappear.

    Within a batch the directions differ.  An *exact* path held by
    another project is a collision, and is refused: see :data:`_Owner`
    for how one batch comes to carry two projects for one destination.
    Ancestor and descendant reservations in the same batch are the
    supported nested arrangement and stay allowed.  Outside a batch the
    answer is ``None``, there being no generation to tell apart.
...
def claim_target_path(path: Path, project: str) -> None:
    ...
    conflict resolution and is about to clone into it.  Re-reserving is
    how a retry continues, so the same project asking again for a path
    it holds is answered rather than refused -- unless its batch has
    since been abandoned.  Outside a batch nothing is reserved and
    nothing refused: with no generation there is no batch to attribute
    the reservation to, and so no claim that the path is ours to delete.
    ...
    Raises:
        TargetOwnedError: If another clone holds the reservation,
            ...
            has been abandoned -- whether the reservation is being
            taken now or was already held.
    """
    generation = current_generation()
    if generation is None:
        return
    ...
    if held is None:
        _refuse_if_abandoned(path, owner, just_taken=True)
        return
    ...
    if held == owner:
        _refuse_if_abandoned(path, owner, just_taken=False)
        return
...
def _refuse_if_abandoned(path: Path, owner: _Owner, *, just_taken: bool) -> None:
    """Refuse a reservation whose batch has been abandoned.

    Asked both when a reservation is taken and when one already held is
    asked for again -- a retry, or the pre-clone checks that follow
    conflict resolution.  Answering the second without asking let a
    worker whose batch had given up go on to create directories and
    write nested protection into a parent repository.

    What becomes of the reservation differs.  One *just taken* can have
    missed the batch-wide release, a worker being able to reach its
    claim after that has run, so it is given straight back -- or it
    would stay blocked for the rest of the process.  Checking after
    publishing makes that safe in either order: the release takes the
    entry if it runs second, and this removes it if it ran first.  One
    *already held* predates the abandonment, so the batch's own
    lifecycle accounts for it -- the release, or the hold-back for a
    worker still running -- and removing it here could hand a path this
    worker has part-written to a later batch.
    ...
    Args:
        ...
        owner: Reservation recorded for it.
        just_taken: Whether this call published it.

    Raises:
        TargetOwnedError: If the batch has been abandoned.
    """
    ...
    raise TargetOwnedError(f"{path} is reserved for a batch that has been abandoned")
...
def claim_new_target(path: Path, project: str) -> None:
    ...
    Raises:
        TargetOwnedError: If the destination is not this clone's,
            cannot be inspected to find out, or belongs to a batch that
            has been abandoned.
    """
    generation = current_generation()
    if generation is None:
    ...
    if held == (generation, project):
        # This clone's own work, from an earlier attempt -- unless the
        # batch has since given up on it.
        _refuse_if_abandoned(path, (generation, project), just_taken=False)
        return
    raise TargetOwnedError(
...
def hold_back_claim(
    ...
    Args:
        ...
        before_release: Run against that future on completion, while
            the reservation is still held -- the one safe moment: before
            it the worker may be writing, after it the path may be
            somebody else's.  Its failure cannot strand the reservation.
    """
    with _claimed_lock:
