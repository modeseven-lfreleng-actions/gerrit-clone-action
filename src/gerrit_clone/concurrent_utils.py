...
class _TrackedThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def abandon(self) -> int:
        ...
        Marks the batch abandoned, cancels every future that has not
        started, terminates the child processes that running tasks
        registered, and marks the executor so that leaving
        :func:`interruptible_executor` does not wait for those tasks to
        finish.
        ...
        # Published before the cancel, not after.  A future can start
        # while the cancel is walking the set, and its worker must find
        # the batch abandoned from its first step: otherwise it passes
        # the pre-clone checks and creates directories before the only
        # thing left to stop it -- the refusal of its child -- applies.
        refuse_generation(self._generation)
...
def handle_sigint_gracefully() -> None:
    ...
    SIGTERM is handled alongside it, for the reason given in
    :func:`handle_sigterm_gracefully`.
...
def handle_sigterm_gracefully() -> None:
    """Stop tracked children before SIGTERM ends the process.

    Clone children run in their own session, so that terminating one
    reaches the ``ssh`` and ``git-remote-https`` helpers git spawns.
    The cost is that they no longer share the CLI's process group: a
    SIGTERM delivered to that group -- by a supervisor, a CI runner
    cancelling a job, or ``kill`` on the shell's job -- reaches Python
    but not them, and they would carry on after it exited.

    The handler signals the children, then restores the default
    disposition and re-raises, so the process still dies of SIGTERM and
    reports the exit status a caller expects.
    """

    def signal_handler(signum: int, frame: object) -> None:  # noqa: ARG001
        stopped = terminate_tracked_children()
        if stopped:
            logger.warning(f"Terminated {stopped} child process(es) on SIGTERM")
