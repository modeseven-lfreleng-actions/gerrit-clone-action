...
from gerrit_clone import subprocess_tracking
...
class TestRunTracked:
    ...
    def test_nothing_the_child_leaves_in_its_group_survives(self) -> None:
        """The leader exiting does not prove its group empty.

        A helper that lets go of the inherited pipes and ignores SIGTERM
        outlives git there, and once the child is untracked no abandon
        or SIGTERM would have a record with which to reach it.
        """
        helper = (
            "import signal, time; "
            "signal.signal(signal.SIGTERM, signal.SIG_IGN); "
        ...
        leader = (
            ...
            "time.sleep(0.5); print(os.getpgrp())"
        )

        result = run_tracked([sys.executable, "-c", leader])
        group = int(result.stdout)
    ...
    def test_an_interrupted_wait_stops_the_child(self) -> None:
        """A Ctrl+C on the main thread lands in ``communicate()``.

        The child is in its own session, out of reach of the terminal's
        SIGINT, so it has to be stopped here as ``subprocess.run`` would
        stop it -- or the interrupt leaves it running.
        """
        real = subprocess.Popen.communicate
        interrupted: list[subprocess.Popen[str]] = []
        ...
        try:
            ...
            assert interrupted, "the wait was never reached"
            with pytest.raises(ProcessLookupError):
                os.killpg(interrupted[0].pid, 0)
            assert not _tracked
        finally:
    ...
    def test_an_interrupted_launch_does_not_orphan_its_child(self) -> None:
        """A Ctrl+C can land while the launch is still under way.

        The launch runs on a thread of its own and is not cut short with
        the wait, so the child it goes on to create would otherwise have
        nobody left to stop it or take it out of the registry.
        """
        under_way = threading.Event()
        release = threading.Event()
        launched: list[subprocess.Popen[str]] = []
        interrupted: list[bool] = []
        real_start = subprocess_tracking._start_tracked_child
        real_result = Future.result

        def gated_start(*args: Any) -> tuple[subprocess.Popen[str], int | None]:
            under_way.set()
            release.wait(timeout=PROMPT_SECONDS)
            process, group = real_start(*args)
        ...
        def result(self: Future[Any], timeout: float | None = None) -> Any:
            if not interrupted:
                ...
                assert under_way.wait(timeout=PROMPT_SECONDS)
                raise KeyboardInterrupt
            return real_result(self, timeout)

        raised = False
        with (
            patch.object(subprocess_tracking, "_start_tracked_child", gated_start),
        ...
        # Only now does the launch go on to create its child.
        release.set()
        try:
            assert raised, "the interrupt did not propagate"
            deadline = time.monotonic() + PROMPT_SECONDS
            while time.monotonic() < deadline and (not launched or _tracked):
                time.sleep(0.05)
            assert launched, "the launch never completed"
            with pytest.raises(ProcessLookupError):
...
class TestTerminateAll:
    """The shared escalation, driven directly with scripted groups."""

    @staticmethod
    @contextlib.contextmanager
    def _scripted(
        alive: Callable[[object, int | None], bool],
        stop: Callable[[object, int | None], None],
        kill: Callable[[object, int | None], None],
    ) -> Generator[None, None, None]:
        """Replace every probe and signal, with a short grace period."""
        with (
            patch("gerrit_clone.process_groups._stop_group", side_effect=stop),
            patch("gerrit_clone.process_groups._group_is_alive", side_effect=alive),
            patch("gerrit_clone.process_groups._kill_group", side_effect=kill),
            patch("gerrit_clone.process_groups._TERMINATE_GRACE_SECONDS", 0.2),
            patch("gerrit_clone.process_groups._POLL_INTERVAL_SECONDS", 0.01),
        ):
            yield

    def test_a_group_seen_gone_is_never_signalled_again(self) -> None:
        """An empty group's id is free for reuse.

        Once a group has been seen to go, a later probe answering
        "alive" is some other group that took over the id, and killing
        it as this batch's survivor would reach well beyond the batch.
        """
        reused, stubborn = 101, 202
        probes = {reused: 0, stubborn: 0}
        killed: list[int | None] = []

        def alive(_process: object, group: int | None) -> bool:
            assert group is not None
            probes[group] += 1
            if group == reused:
                # Gone at the first probe; its id is taken over after.
                return probes[group] > 1
            return group not in killed

        def kill(_process: object, group: int | None) -> None:
            killed.append(group)

        groups: list[tuple[subprocess.Popen[str], int | None]] = [
            (cast("subprocess.Popen[str]", MagicMock()), reused),
            (cast("subprocess.Popen[str]", MagicMock()), stubborn),
        ]
        with self._scripted(alive, stop=lambda *_: None, kill=kill):
            terminate_all(groups)

        assert killed == [stubborn]
        assert probes[reused] == 1

    def test_a_group_another_escalation_saw_go_is_left_alone(self) -> None:
        """Two escalations can hold the same child's recorded group.

        An abandon and the SIGTERM handler, say.  Once one has seen the
        group go, the other must not take a later "alive" -- the id
        reused -- as a survivor of its own to signal.
        """
        probes = 0
        signalled: list[str] = []

        def alive(_process: object, _group: int | None) -> bool:
            nonlocal probes
            probes += 1
            # Present for the first signal, gone at the next probe, and
            # its id taken over after that.
            return probes != 2

        groups: list[tuple[subprocess.Popen[str], int | None]] = [
            (cast("subprocess.Popen[str]", MagicMock()), 303),
        ]
        with self._scripted(
            alive,
            stop=lambda *_: signalled.append("TERM"),
            kill=lambda *_: signalled.append("KILL"),
        ):
            terminate_all(groups)
            terminate_all(groups)
    ...
    def test_a_group_gone_before_the_first_signal_is_not_signalled(self) -> None:
        """The group is recorded at launch, not when escalation begins.

        A child can exit between the snapshot and the first SIGTERM,
        after which its id is free for reuse -- so even that first
        signal must follow a probe that found the group present.
        """
        signalled: list[str] = []
        groups: list[tuple[subprocess.Popen[str], int | None]] = [
            (cast("subprocess.Popen[str]", MagicMock()), 505),
        ]
        with self._scripted(
    ...
    def test_a_group_that_empties_before_its_signal_is_marked_gone(self) -> None:
        """The signal itself can be the first to learn the group has gone.

        Its id is free for reuse from then on, so a later probe answering
        "present" would be another group -- which must not be killed as
        this one's survivor.
        """
        probes = 0
        signals: list[int] = []
        ...
        groups: list[tuple[subprocess.Popen[str], int | None]] = [
            (cast("subprocess.Popen[str]", MagicMock()), 606),
        ]
        with (
            patch("gerrit_clone.process_groups._group_is_alive", side_effect=alive),
            patch("gerrit_clone.process_groups.os.killpg", side_effect=killpg),
            patch("gerrit_clone.process_groups._TERMINATE_GRACE_SECONDS", 0.2),
            patch("gerrit_clone.process_groups._POLL_INTERVAL_SECONDS", 0.01),
        ):
            terminate_all(groups)
    ...
    def test_a_group_cannot_be_marked_gone_while_it_is_signalled(self) -> None:
        """The probe and the signal it permits are one step.

        Otherwise a concurrent escalation could see the group go in
        between, and the signal would reach whatever took over its id.
        """
        process = cast("subprocess.Popen[str]", MagicMock())
        rivals: list[threading.Thread] = []
        marked_before_signal: list[bool] = []
        probes = 0

        def alive(_process: object, _group: int | None) -> bool:
            nonlocal probes
            probes += 1
            # Present for this escalation's signal, gone for the rival.
            return probes == 1

        def stop(_process: object, _group: int | None) -> None:
            # Another escalation sees the group go while this one is
            # between deciding to signal and signalling.
            rival = threading.Thread(target=_still_alive, args=([(process, 404)],))
            rivals.append(rival)
            rival.start()
            rival.join(timeout=0.3)
            marked_before_signal.append(not rival.is_alive())

        with self._scripted(alive, stop=stop, kill=lambda *_: None):
            terminate_all([(process, 404)])
            for rival in rivals:
                rival.join(timeout=PROMPT_SECONDS)

        assert marked_before_signal == [False]


class TestExecutorExit:
    """Leaving the block must not wait for work that was abandoned."""

    def test_the_batch_is_refused_before_pending_work_is_cancelled(self) -> None:
        """A future can start while the cancel walks the set.

        Its worker must already find the batch abandoned, or it gets
        through the pre-clone checks and writes to disk before anything
        refuses it.
        """
        seen: list[bool] = []
        with interruptible_executor(max_workers=1, thread_name_prefix="test") as ex:
            real_cancel = ex.cancel_all_pending

            def cancel() -> int:
                seen.append(generation_abandoned(ex.generation))
                return real_cancel()

            with patch.object(ex, "cancel_all_pending", side_effect=cancel):
                ex.abandon()
