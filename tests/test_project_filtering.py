...
class TestMatchProjectPattern:
    ...
    def test_exact_match_with_slashes(self) -> None:
        assert match_project_pattern("testsuite/pythonsdk-tests", "testsuite/pythonsdk-tests") is True
...
class TestNormalizeProjectList:
    """Tests for the _normalize_project_list helper."""

    def test_comma_separated(self) -> None:
        result = _normalize_project_list(["ccsdk,oom,cps"])
    ...
    def test_multiple_entries(self) -> None:
        result = _normalize_project_list(["ccsdk,oom", "cps", "aai"])
        assert result == ["ccsdk", "oom", "cps", "aai"]

    def test_deduplication_preserves_order(self) -> None:
        result = _normalize_project_list(["ccsdk,oom,ccsdk,oom"])
        assert result == ["ccsdk", "oom"]

    def test_strips_whitespace(self) -> None:
        result = _normalize_project_list(["  ccsdk  ,  oom  "])
        assert result == ["ccsdk", "oom"]

    def test_drops_empties(self) -> None:
        result = _normalize_project_list(["", "  ", ",,,"])
        assert result == []
    ...
    def test_wildcards_preserved(self) -> None:
        result = _normalize_project_list(["*sdk*, testsuite/*"])
...
class TestFilterProjects:
    ...
    def test_include_wildcard(self) -> None:
        """Wildcard *sdk* matches 'ccsdk' (contains 'sdk') and
        'testsuite/pythonsdk-tests'.  Since 'ccsdk' is matched, hierarchical
        matching does NOT apply for wildcard patterns — only fnmatch is used.
        But fnmatch('ccsdk/apps', '*sdk*') is also True because the full
        string contains 'sdk', so all ccsdk/* children are included too."""
        result = filter_projects(self.PROJECTS, include_patterns=["*sdk*"])
        names = {p.name for p in result}
        # fnmatch matches any name containing "sdk" anywhere
        assert names == {
            "ccsdk",
            "ccsdk/apps",           # not via hierarchy — via fnmatch (contains "sdk")
            "ccsdk/features",       # not via hierarchy — but does NOT contain "sdk"
            "ccsdk/features/test",  # not via hierarchy — but does NOT contain "sdk"
            "testsuite/pythonsdk-tests",  # contains "sdk"
        }
        # Actually: fnmatch("ccsdk/features", "*sdk*") is True because
        # the substring "sdk" is present in "ccsdk/features".  Same for
        # "ccsdk/features/test".  So ALL ccsdk/* names match.
    ...
    def test_exclude_wildcard(self) -> None:
        ...
        assert "ccsdk/features/test" not in names
        assert "testsuite/pythonsdk-tests" not in names
        # "testsuite" itself does not contain "test" in the fnmatch sense
        # because fnmatch is anchored — let's verify:
        # fnmatch("testsuite", "*test*") → True (it does contain "test")
...
class TestConfigProjectNormalization:
    ...
    def test_exclude_comma_separated(self) -> None:
        cfg = self._make_config(exclude_projects=["testsuite/pythonsdk-tests, other/bad-repo"])
