...
from gerrit_clone.github_clone_url import UnsafeCloneUrlError
...
class TestGhCloneCommand:
    ...
    def test_a_credentialed_url_is_refused(self) -> None:
        ...
        config = Config(
            host="github.com/org", source_type=SourceType.GITHUB, path=Path("/tmp")
        )

        with pytest.raises(UnsafeCloneUrlError, match="reach the git command line"):
            _build_gh_clone_command(project, config, Path("/tmp/repo"))

    def test_a_credentialed_url_is_reported_not_raised(self) -> None:
        ...
        result = clone_with_gh_cli(
            project, config, Path("/tmp/repo"), datetime.now(UTC)
        )

        assert result.status == CloneStatus.FAILED
        assert result.error_message is not None

    def test_an_ordinary_url_is_used_as_the_identifier(self) -> None:
        project = Project(
            name="org/repo",
            state=ProjectState.ACTIVE,
            source_type=SourceType.GITHUB,
        ...
        cmd = _build_gh_clone_command(project, config, Path("/tmp/repo"))
    ...
    def test_a_project_without_a_url_falls_back_to_its_name(self) -> None:
...
class TestCloneGitHubRepository:
    ...
    @patch("gerrit_clone.github_gh_cli.run_tracked")
    ...
    def test_a_credentialed_project_url_is_not_written_back(
        ...
        with patch("gerrit_clone.github_token_hygiene.run_tracked") as mock_run:
